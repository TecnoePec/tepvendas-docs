...
| set/2026 | `AIzaSyDZ3Vj…` (Firebase Web API Key, projeto Firebase `tepvenda`, uid `71fadeaa…`) | Hardcoded em `FirebaseStorage(Delete)Service.cs:23` | ✅ (a) Backend .NET migrado de signInAnonymously + Firebase.Storage → Google.Cloud.Storage.V1 com a Service Account do projeto (task-def rev 78), (b) chave **deletada** via `gcloud services api-keys delete` 2026-09-24 — smoke test confirma HTTP 400 "API key expired" |
